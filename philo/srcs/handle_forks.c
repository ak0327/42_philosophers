...
/*   Updated: 2023/02/20 11:19:30 by takira           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	pthread_mutex_lock(&params->lock_state);
	...
	if (is_philo_can_eating(params, philo->idx))
...
//	printf("  [#Put] %zu-4 put  %zu\n", philo->idx, philo->second_take);
	...
	&& is_philo_can_eating(params, philo->left_philo_idx))
	{
	...
	&& is_philo_can_eating(params, philo->right_philo_idx))
	{
